...
def create_controllers(net, ds):
    ...
    for k, value in enumerate(num_loads3):
        if value:
            ConstControl(net, element="load", variable="scaling", element_index=[k],
                         data_source=ds, profile_name=["loads"])
    for l, value in enumerate(num_sgen3):
        if value:
            ConstControl(net, element="sgen", variable="scaling", element_index=[l],
                         data_source=ds, profile_name=["sgens"])
    return net

...
def create_output_writer(net, buses_area3, time_steps, output_dir):
    ...
    dennis_net = MyGrid(net, 2)
    #ow.log_variable("res_line", "loading_percent", index=dennis_net.get_indices("line"), eval_function=np.max, eval_name="Max. Leitungsauslastung Dennis")
    ow.log_variable("res_bus", "vm_pu", index=dennis_net.get_indices("bus"), eval_function=np.max, eval_name="Max. Spannungspegel Dennis")
    ow.log_variable("res_bus", "vm_pu", index=dennis_net.get_indices("bus"), eval_function=np.min, eval_name="Min. Spannungspegel Dennis")
...
output_dir = r"C:\Users\User\Documents\Uni\Master\1. Semester\Pandapower\Hausarbeit\Zeitreihen_4.5\Output dir"

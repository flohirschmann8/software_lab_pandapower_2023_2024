...
from tap_control import TapController
...
# setting up Lukas controller
# setting Lukas ext_grid id
lukas_area = 1
...
Lukas_subgrid = MyGrid(whole_net=grid,ext_grid_subgrid=lukas_area)
...
ow.log_variable(table="res_bus", variable="vm_pu", index=Lukas_subgrid.get_indices(element="bus"), eval_function=np.max, eval_name="Lukas subgrid max. voltage")
ow.log_variable(table="res_bus", variable="vm_pu", index=Lukas_subgrid.get_indices(element="bus"), eval_function=np.min, eval_name="Lukas subgrid min. voltage")
ow.log_variable(table="res_line", variable="loading_percent", index=Lukas_subgrid.get_indices(element="line"), eval_function=np.max, eval_name="Lukas subgrid max. line loading")
